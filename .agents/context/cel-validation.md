...
- **Runtime errors** — non-cost-limit eval errors (type mismatches, null-comparison, missing keys) **fail the write** with a `cel_runtime_error` FailedRule, so a broken rule fails closed and the author gets immediate feedback rather than silent non-enforcement. This is the shipped default (see commit `8226bc8`); `Eval` keeps a `WithLenientRuntimeErrors` opt-in that downgrades them to logged soft errors, but no config path enables it. Cost-limit exceedance hard-fails in both modes so operators can flag DoS attempts. The most common runtime mismatch — operand/operator type errors — is now caught at `ImportSchema` by lint rule 3's type pass (see below), so a fail-closed default does not surprise authors at first write.
...
3. **Field references resolve, with type checking.** Every `self.<path>` traversal must terminate at a real field in the schema being imported (path half), and the operand/operator types must be compatible (type half). Both run in `internal/schema/cel/lint.go`: the path half walks the AST collecting `self.*` chains and checks them against the declared field set; the type half (`lint_types.go`) walks the same AST, types each `self.<path>` leaf via `selfDescriptor`, and rejects mismatches such as a number-vs-string comparison or a string method on a numeric field.

   The env exposes `self` as `dyn` (a typed env would need a custom `cel.TypeProvider` for the nested `self` object), so the type half is a hand-rolled AST pass rather than cel-go's static checker. It deliberately mirrors cel-go's **runtime** coercion — numeric cross-type (`int`/`uint`/`double`) compares are allowed and heterogeneous `==`/`!=` is never flagged, matching what the evaluator accepts — so a rule lints clean iff it also evaluates without a type error. False negatives are acceptable (anything not modelled infers as `dyn` and passes); false positives are not.
...
Rule 3's type matching was originally specced to ride on a **typed CEL env** (`cel.Variable("fields.payments.fee", cel.DoubleType)`). That was not implemented: typing the nested `self` object needs a custom `cel.TypeProvider`, so the env stays `dyn` and the type check is the hand-rolled AST pass described above (`lint_types.go`). Either way the goal holds — type mismatches are caught at schema import, not at the first write.
