...
const queryAuditWriteLogKeyset = `-- name: QueryAuditWriteLogKeyset :many
SELECT id, tenant_id, actor, action, field_path, old_value, new_value, config_version, metadata, created_at, object_kind, previous_hash, entry_hash, chain_epoch FROM audit_write_log
WHERE ($1::UUID IS NULL OR tenant_id = $1)
  AND (NULLIF($2::TEXT, '') IS NULL OR actor = $2)
  AND (NULLIF($3::TEXT, '') IS NULL OR field_path = $3)
...
type QueryAuditWriteLogKeysetParams struct {
	Column1 pgtype.UUID        `json:"column_1"`
	Column2 string             `json:"column_2"`
	Column3 string             `json:"column_3"`
	Column4 pgtype.Timestamptz `json:"column_4"`
...
func (q *Queries) QueryAuditWriteLogKeyset(ctx context.Context, arg QueryAuditWriteLogKeysetParams) ([]AuditWriteLog, error) {
	rows, err := q.db.Query(ctx, queryAuditWriteLogKeyset,
		arg.Column1,
		arg.Column2,
		arg.Column3,
		arg.Column4,
